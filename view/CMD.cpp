...
void printOne(std::string const &book_id, std::string const &name, std::string const &author, int book_count,
              int borrow_count) {
    printf("|%-9s|%-6s|%-8s|%-12d|%-15d|\n", book_id.c_str(), name.c_str(), author.c_str(), book_count, borrow_count);
}

void printGap() {
    printf("+%s+%s+%s+%s+%s+\n", std::string(9, '-').c_str(), std::string(6, '-').c_str(), std::string(8, '-').c_str(),
           std::string(12, '-').c_str(), std::string(15, '-').c_str());
}
